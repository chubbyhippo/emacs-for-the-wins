...
#
#   ./install-31.sh              do everything: deps, clone, build, portable
#   ./install-31.sh --list       show resolved settings and exit
#   ./install-31.sh --skip-deps  skip the pacman install step
#   ./install-31.sh --skip-clone reuse an existing checkout at $SRC_DIR
#   ./install-31.sh --skip-build reuse an existing build at $SRC_DIR/build
#   ./install-31.sh --skip-portable skip the portability (DLL-closure) step
#   ./install-31.sh --skip-fixups   skip auto-applying known upstream fixes
#   ./install-31.sh --only-portable just (re)run the portability step
#   ./install-31.sh --branch emacs-31 --repo <url> --jobs 8 --prefix <dir>
#
# Must be run from an MSYS2 "mingw64.exe" shell.
#
# Known-fix, auto-applied: this Emacs snapshot's treesit.c still resolves
# the deprecated ts_language_version symbol at runtime, but MSYS2's
# mingw-w64-x86_64-libtree-sitter (>=0.25, current is 0.26.x) dropped it for
# ts_language_abi_version — the DLL loads fine standalone, but Emacs's
...
# ts_language_abi_version; this Emacs snapshot's treesit.c still wants the
...
fix_tree_sitter_symbol() {
    ...
    info "MSYS2 libtree-sitter lacks ts_language_version (needed by this Emacs snapshot) — fetching a 0.24.7 build that still exports it"
    ts_tmp=$(mktemp -d)
